...
struct weapon
{
    ...
    int quantity; // Number of dice. ie, 5d10 or 2d10 or whatever.
...
struct items {
    ...
    int weaptype; // Integer referring to spot on armor table. 0 = not weapon
    int armortype; // Armor slot. > 0 = array position on armortype. Surely there's a better way to do this?
    // I feel like there needs to be more here but cannot think of anything else.
};

struct items stanit[typetotal]; // Standard item table. Master list of all items in game.
